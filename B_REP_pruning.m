...
val_size1 = floor(length(monk1_train) * validation_size);
[prune_set1, index1] = datasample(monk1_train, val_size1, 'Replace', false);
monk1_train(index1, :) = [];

val_size2 = floor(length(monk2_train) * validation_size);
[prune_set2, index2] = datasample(monk2_train, val_size2, 'Replace', false);
monk2_train(index2, :) = [];

val_size3 = floor(length(monk3_train) * validation_size);
...
if isequal(monk1_tree_pruned, monk1_tree)
    ...
    fprintf("Unpruned tree accuracy %0.15f\n", acc1);
    ...
    fprintf("Pruned tree accuracy %0.15f   |   Unpruned tree accuracy %0.15f\n",acc1_prune,acc1);
end
